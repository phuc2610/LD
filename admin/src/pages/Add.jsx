  ...
  const onSubmitHandler = async(e) => {
    ...
    try {
      ...
      if(response.data.success){
        ...
        setColors([]);
        setType("T-SHIRTS");
        setStockBySize({S:0,M:0,L:0,XL:0});
      } else {
  ...
  return (
    <div>
        <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-4'>
            <div>
              ...
              <div className='flex gap-2 flex-wrap'>
                <label htmlFor="image1">
                  <img className='w-20 h-20 object-cover cursor-pointer' src={!image1 ? assets.upload_area : URL.createObjectURL(image1)} alt="" />
                  <input onChange={(e) => setImage1(e.target.files[0])} type="file" id="image1" hidden />
                </label>
                <label htmlFor="image2">
                  <img className='w-20 h-20 object-cover cursor-pointer' src={!image2 ? assets.upload_area : URL.createObjectURL(image2)} alt="" />
                  <input onChange={(e) => setImage2(e.target.files[0])} type="file" id="image2" hidden />
                </label>
                <label htmlFor="image3">
                  <img className='w-20 h-20 object-cover cursor-pointer' src={!image3 ? assets.upload_area : URL.createObjectURL(image3)} alt="" />
                  <input onChange={(e) => setImage3(e.target.files[0])} type="file" id="image3" hidden />
                </label>
                <label htmlFor="image4">
                  <img className='w-20 h-20 object-cover cursor-pointer' src={!image4 ? assets.upload_area : URL.createObjectURL(image4)} alt="" />
            ...
            <button type='submit' className='w-32 py-3 mt-4 bg-black text-white cursor-pointer rounded-lg hover:bg-gray-700 transition ease-in-out text-sm font-semibold uppercase tracking-wide'>Thêm</button>
